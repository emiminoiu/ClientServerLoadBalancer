...
public class RetrieveServerResponse implements Runnable {
	...
	private void server_Reconnect(String s) {	   
			
			
								
			if(s.startsWith("port"))
			{
			String[] reconnect = s.split("\\:");
			if (reconnect[0].equalsIgnoreCase("port")) {
				reconnect[1] = reconnect[1].trim();
				int port = Integer.parseInt(reconnect[1]);
				System.out.println(port);
				
				parent.connect(port, true);
